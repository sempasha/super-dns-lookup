...
- **Resolve lock** - ability to handle several lookup calls for resolving the same hostname with a single DNS resolve query.
- **Resolve lock timeout** - ability to limit the time of waiting for the current resolve query to be completed before firing a new query.
...
| **Resolve lock**                          | When `LookupController` receives a lookup request for a hostname that needs to be resolved, either because it's not found in the `CacheService` or its TTL has expired, it initiates a resolve request using `ResolverService#resolve4/resolve6`. If another lookup request for the same hostname is received while the previous `ResolverService#resolve4/resolve6` call is still in progress, the controller will wait until the current resolve request is completed and then use the resolve result for both lookup requests, ensuring that only one resolve request is performed at a time for every hostname. The mechanism is known as a **resolve lock** and activated by `LookupController`'s option `resolveLock` which is `true` by default.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Resolve lock timeout**                  | Using the **resolve lock** feature, `LookupController` limits the number of `ResolverService#resolve4/resolve6` calls for the same hostname to one. However, if the original `ResolverService#resolve4/resolve6` call takes a long time to resolve due to temporary network issues, it can cause delays in responses for all `LookupController#lookup` requests for that hostname. To mitigate this, `LookupController` allows spawning a new `ResolverService#resolve4/resolve6` call after waiting for a certain amount of time, known as the **resolve lock timeout**. Newly spawned request will lock the resolve just like the previous one did. Default resolve lock timeout is **one second** and `LookupController` allow to change its value using option `resolveLockTimeoutMs`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
