...
export interface CacheService<Value extends unknown = unknown> {
  /**
   * Provides iterator over `[key, value]` pairs stored in cache.
   *
   * @example
   ...
   * }
   * @returns Iterator over `[key, value]` pairs stored in cache.
   */
  entries(): Iterator<[key: string, value: Value]>;
...
export class LRUCacheService<Value extends unknown = unknown> implements CacheService<Value> {
  ...
  /**
   * Creates cache service with specified options.
   *
   * @example
   ...
   * @returns Iterator over `[key, value]` pairs stored in cache.
   */
  public entries(): Iterator<[key: string, value: Value]> {
