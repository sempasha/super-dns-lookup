import { isIPv4, isIPv6 } from 'node:net';

/**
 * IsIpService is a simple IP address recognition service.
 * It tells whether provided value is IPv4 or IPv6 address or not an IP at all.
 * Basically it does the same as dns.isIPv4 and dns.isIPv6 do.
 *
 * @example
 * import { IsIpService } from 'super-and-lookup';
 * import ipRegex from 'ip-regex';
 *
 * export class IsIpServiceExample implements IsIpService {
 *   public isIPv4(string: string) {
 *     return ipRegex.v4.test(string);
 *   }
 *
 *   public isIPv6(string: string) {
 *     return ipRegex.v6.test(string);
 *   }
 * }
 * @see https://nodejs.org/docs/latest/api/net.html#netisipv4input
 * @see https://nodejs.org/docs/latest/api/net.html#netisipv6input
 */
export interface IsIpService {
  /**
   * Should tell whether provided string is IPv4 address or not.
   * Should behave the same way NodeJS's [net.isIPv4](https://nodejs.org/docs/latest/api/net.html#netisipv4input) does.
   *
   * @example
   * import { equal } from 'node:assert';
   * import { IsIpServiceExample } from './is-ip-service-example';
   *
   * const isIpService = new IsIpServiceExample();
   * equal(isIpService.isIPv4('127.0.0.1'), true);
   * equal(isIpService.isIPv4('::1'), false);
   * @see https://nodejs.org/docs/latest/api/net.html#netisipv4input
   * @param string Value to check
   * @return Only returns true when provided string is an IPv4 address string
   */
  isIPv4(string: string): boolean;

  /**
   * Should tell whether provided string is IPv4 address or not.
   * Should behave the same way NodeJS's [net.isIPv6](https://nodejs.org/docs/latest/api/net.html#netisipv6input) does.
   *
   * @example
   * import { equal } from 'node:assert';
   * import { IsIpServiceExample } from './is-ip-service-example';
   *
   * const isIpService = new IsIpServiceExample();
   * equal(isIpService.isIPv6('::1'), true);
   * equal(isIpService.isIPv6('127.0.0.1'), false);
   * @see https://nodejs.org/docs/latest/api/net.html#netisipv6input
   * @param string Value to check
   * @return Only returns true when provided string is an IPv6 address string
   */
  isIPv6(string: string): boolean;
}

/**
 * Implementation of IsIpService using NodeJS net.isIPv4 and net.isIPv6 functions.
 *
 * @example
 * const isIpService = new NodeIsIpService();
 * const lookupController = new LookupController({ isIpService });
 * @see https://nodejs.org/docs/latest/api/net.html#netisipv4input
 * @see https://nodejs.org/docs/latest/api/net.html#netisipv6input
 */
export class NodeIsIpService implements IsIpService {
  /**
   * Should tell whether provided string is IPv4 address or not.
   * Should behave the same way NodeJS's [net.isIPv4](https://nodejs.org/docs/latest/api/net.html#netisipv4input) does.
   *
   * @example
   * import { equal } from 'node:assert';
   * import { NodeIsIpService } from 'super-dns-lookup';
   *
   * const isIpService = new NodeIsIpService();
   * equal(isIpService.isIPv4('127.0.0.1'), true);
   * equal(isIpService.isIPv4('::1'), false);
   * @see https://nodejs.org/docs/latest/api/net.html#netisipv4input
   * @param string Value to check
   * @return Only returns true when provided string is an IPv4 address string
   */
  public readonly isIPv4 = isIPv4;

  /**
   * Should tell whether provided string is IPv4 address or not.
   * Should behave the same way NodeJS's [net.isIPv6](https://nodejs.org/docs/latest/api/net.html#netisipv6input) does.
   *
   * @example
   * import { equal } from 'node:assert';
   * import { NodeIsIpService } from 'super-dns-lookup';
   *
   * const isIpService = new NodeIsIpService();
   * equal(isIpService.isIPv6('::1'), true);
   * equal(isIpService.isIPv6('127.0.0.1'), false);
   * @see https://nodejs.org/docs/latest/api/net.html#netisipv6input
   * @param string Value to check
   * @return Only returns true when provided string is an IPv6 address string
   */
  public readonly isIPv6 = isIPv6;
}
